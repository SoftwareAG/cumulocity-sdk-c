...
static const char *objfmt = "{\"name\":\"%s\",\"type\":\"%s\"}";
...
{
        curl_formadd(first, last, CURLFORM_COPYNAME, "object",
                     CURLFORM_PTRCONTENTS, obj, CURLFORM_END);
        curl_formadd(first, last, CURLFORM_COPYNAME, "filesize",
                     CURLFORM_PTRCONTENTS, fz, CURLFORM_END);
}
...
SrNetBinHttp::~SrNetBinHttp() {curl_slist_free_all(chunk);}
